...
type ComparisonStatus = boolean | 'limited' | 'medium';

type ComparisonItem = {
  feature: string;
  custom: ComparisonStatus;
  platform: ComparisonStatus;
  description: string;
};
          ...
          <div className="brand-card overflow-hidden">
            ...
            <div className="divide-y divide-brand-gray-dark">
              {comparisonData.map((item) => (
                <div key={item.feature} className="grid grid-cols-4 gap-4 p-6 hover:bg-white/5 transition-colors duration-300">
                  <div className="col-span-2 flex flex-col justify-end space-y-1 min-h-[60px]">
                    <h4 className="font-inter font-semibold text-white leading-snug">{item.feature}</h4>
                    <p className="font-inter text-sm text-brand-gray-light leading-relaxed">{item.description}</p>
                  </div>
                  <div className="flex justify-center items-center">
                    <StatusIcon status={item.custom} />
                  </div>
                  <div className="flex justify-center items-center">
                    <StatusIcon status={item.platform} />
                  </div>
                </div>
              ))}
            </div>
